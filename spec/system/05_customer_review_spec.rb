...
describe '[STEP5] 顧客レビュー一覧のテスト' do
  ...
  describe '投稿画面のテスト' do
    ...
    context '投稿成功のテスト' do
      before do
        fill_in 'review[value]', with: Faker::Number.between(from: 1, to: 5)
      ...
      it '正しく投稿される' do
        expect { click_button '投稿する' }.to change(Review.all, :count).by(1)
      end
      it '投稿後のリダイレクト先が、投稿完了画面になっている' do
        click_button '投稿する'
        expect(current_path).to eq '/public/reviews/complete'
      end
    end
  end
end